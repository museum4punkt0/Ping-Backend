...
LANGUEAGE_STYLE_CHOICES = Choices(
    ("easy", "Easy"), ("fun", "Fun"), ("kids", "Kids"), ("professional", "Professional")
)
...
def get_image_path(instance, filename):
    ...
    if instance.__class__.__name__ in ("Users", "Collections"):
    ...
    elif instance.__class__.__name__ in (
        "ObjectsItem",
        "MuseumsImages",
        "ObjectsImages",
        "PredefinedAvatars",
        ...
        if instance.__class__.__name__ in (
            "ObjectsItem",
            "ObjectsImages",
            "ObjectsLocalizations",
        ):
...
def validate_percent(value):
    if value < 0 or value > 100:
        raise ValidationError("Value must be between 0-100", params={"value": value})
...
class Users(models.Model):
    ...
    positionx = models.DecimalField(
        db_column="positionX", max_digits=4, decimal_places=0, default=0
    )
    positiony = models.DecimalField(
        db_column="positionY", max_digits=4, decimal_places=0, default=0
    )
    floor = models.IntegerField(blank=True, null=True, default=0)
    language = models.CharField(
        max_length=45, choices=LOCALIZATIONS_CHOICES, default=LOCALIZATIONS_CHOICES.en
    )
    sync_id = models.UUIDField(default=uuid.uuid4, editable=False)
    synced = models.BooleanField(default=False)
    user_level = models.CharField(max_length=45, choices=LEVELS_CHOICES, default=1)
...
class Settings(models.Model):
    ...
    languages = MultiSelectField(
        choices=LOCALIZATIONS_CHOICES, max_choices=2, max_length=20
    )
    language_styles = MultiSelectField(
        choices=LANGUEAGE_STYLE_CHOICES, max_choices=4, max_length=80
    )
    ...
    recognition_threshold = models.SmallIntegerField(
        default=50, validators=[validate_percent]
    )
...
class PredefinedAvatars(models.Model):
    ...
    image = models.ImageField(
        upload_to=get_image_path, blank=True, null=True, max_length=150
    )
...
class MuseumLocalization(models.Model):
    ...
    def clean(self):
        ...
        validate_title = (
            MuseumLocalization.objects.filter(title=title)
            .exclude(museum=museum)
            .exists()
        )
        validate_language = (
            MuseumLocalization.objects.filter(language=language, museum=museum)
            .exclude(id=id)
            .exists()
        )
...
class MusemsTensor(models.Model):
    ...
    tensor_flow_model = models.FileField(
        upload_to="tensor_model/", blank=True, null=True, max_length=150
    )
    tensor_flow_lables = models.FileField(
        upload_to="tensor_label/", blank=True, null=True, max_length=150
    )
    mobile_tensor_flow_model = models.FileField(
        upload_to="tensor_model/", blank=True, null=True, max_length=150
    )
    mobile_tensor_flow_lables = models.FileField(
        upload_to="tensor_label/", blank=True, null=True, max_length=150
    )
    tensor_status = models.CharField(
        max_length=145, choices=TENSOR_STATUSES, default="none"
    )
    mobile_tensor_status = models.CharField(
        max_length=145, choices=TENSOR_STATUSES, default="none"
    )
    sync_id = models.UUIDField(default=uuid.uuid4, editable=False)
    synced = models.BooleanField(default=False)
...
class ObjectsItem(models.Model):
    ...
    priority = models.IntegerField(
        choices=PRIORITY_CHOICES, default=PRIORITY_CHOICES.first
    )
    museum = models.ForeignKey(Museums, models.PROTECT)
    floor = models.IntegerField()
    positionx = models.DecimalField(
        db_column="positionX", max_digits=4, decimal_places=0
    )
    positiony = models.DecimalField(
        db_column="positionY", max_digits=4, decimal_places=0
    )
    vip = models.BooleanField(default=False)
    author = models.CharField(max_length=145, blank=True, null=True)
    language_style = models.CharField(
        max_length=45, choices=LANGUEAGE_STYLE_CHOICES, default="easy"
    )
    avatar = models.ImageField(
        ...
        null=True,
        max_length=150,
        verbose_name="full_image",
    )
    cropped_avatar = models.ImageField(
        upload_to=get_image_path,
        blank=True,
        null=True,
        max_length=150,
    ...
    semantic_relation = models.ManyToManyField(
        "self", through="SemanticRelation", symmetrical=False
    )
    in_tensor_model = models.BooleanField(default=False)
    sync_id = models.UUIDField(default=uuid.uuid4, editable=False)
    synced = models.BooleanField(default=False)
    object_level = models.CharField(max_length=45, choices=LEVELS_CHOICES, default=1)
    created_at = models.DateTimeField(default=timezone.now, editable=False)
    updated_at = models.DateTimeField(default=timezone.now)
...
class MuseumTour(models.Model):
    museum = models.ForeignKey(
        Museums, models.CASCADE, related_name="tours", related_query_name="tours"
    )
    sync_id = models.UUIDField(default=uuid.uuid4, editable=False)
    created_at = models.DateTimeField(default=timezone.now, editable=False)
    updated_at = models.DateTimeField(default=timezone.now)
...
class SemanticRelation(models.Model):
    from_object_item = models.ForeignKey(
        ObjectsItem, on_delete=models.CASCADE, related_name="from_object_item"
    )
    to_object_item = models.ForeignKey(
        ObjectsItem, on_delete=models.CASCADE, related_name="to_object_item"
    )
    sync_id = models.UUIDField(default=uuid.uuid4, editable=False)
    created_at = models.DateTimeField(default=timezone.now, editable=False)
    updated_at = models.DateTimeField(default=timezone.now)
...
def create_objecttensor(sender, instance, **kwargs):
    ...
    ext = name.split(".")[-1]
    if (
        getattr(museum.settings, "save_collections_to_tensor", None)
        and col_image.size > MIN_TENSOR_IMAGE_SIZE
        and ext in ["jpg", "jpeg", "JPG", "JPEG"]
    ):
...
class ObjectsMap(models.Model):
    ...
    objects_item = models.ForeignKey(
        ObjectsItem, related_name="object_map", on_delete=models.CASCADE
    )
    image = models.ImageField()
    sync_id = models.UUIDField(default=uuid.uuid4, editable=False)
    synced = models.BooleanField(default=False)
...
class ObjectsTensorImage(models.Model):
    ...
    objects_item = models.ForeignKey(
        ObjectsItem, related_name="object_tensor_image", on_delete=models.CASCADE
    )
    image = models.ImageField(
        storage=S3Boto3Storage(bucket=settings.AWS_STORAGE_BUCKET_NAME),
        upload_to=get_tensor_image_path,
        max_length=150,
        validators=[
            FileExtensionValidator(allowed_extensions=["jpg", "jpeg", "JPG", "JPEG"])
        ],
    )
    sync_id = models.UUIDField(default=uuid.uuid4, editable=False)
    synced = models.BooleanField(default=False)
    ...
    if ObjectsTensorImage.objects.filter(objects_item=ob_item).count() == 20:
        ...
        try:
            response = s3_client.list_objects(
                ...
                Prefix=f"{museum.sync_id}/temp_dataset",
            )
        except:
            logger.error("Unsuccess images number validation")
        else:
...
class MuseumsImages(models.Model):
    ...
    image_type = models.CharField(
        max_length=45, choices=IMAGE_TYPES, default=IMAGE_TYPES.smpl
    )
            ...
            try:
                ...
                if not check_utf.is_utf8_without_bom(chat):
                    raise ValidationError(
                        "Bad convarsation file encoding. It should be UTF-8"
                    )
...
class SingleLine(models.Model):
    ...
    line_type = models.CharField(
        max_length=45, choices=CHAT_LINE_CHOICES, default=CHAT_LINE_CHOICES.redirect
    )
    ...
    def clean(self):
        ...
        if not redirect and not multichoice:
            raise ValidationError(
                "At least one of redirect or multichoice must be filled"
            )
