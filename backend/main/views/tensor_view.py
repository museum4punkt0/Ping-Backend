...
def recognize(request):
    ...
    if (
        not isinstance(image, (InMemoryUploadedFile, TemporaryUploadedFile))
        or im_format not in ("jpg", "jpeg", "JPEG", "JPG")
        or image.size == 0
    ):
        return JsonResponse(
            {"error": "Image must be jpg format and its size must be not null"},
            safe=True,
        )
    ...
    if user_id:
        try:
            ...
            return JsonResponse(
                {"error": f"User id {user_id} does not exist {e.args}"}, safe=True
            )
    else:
    ...
    if model_obj and label_obj:
        ...
        return JsonResponse(
            {"error": "No tensor model or labels preloaded for museum"}, safe=True
        )
    ...
    if object_id:
        ...
        response = _match_coincidence(
            predict, object_id, img_bytes, museum_name, im_format, obj_name
        )
    else:
