...
def fetch(request):
    ...
    if request.method == "GET":
        ...
        data = {"museums": None, "users": None, "settings": None, "deleted": None}
        ...
        for localizations in museum_localizations:
            localizations = MuseumLocalizationSerializer(
                localizations, fields=FETCH_FIELDS
            ).data
        ...
        for category in categories:
            ...
            category_table = {"localizations": [], "sync_id": None, "updated_at": None}
        ...
        settings_table = {"sync_id": None, "updated_at": None}

        settings_table["sync_id"] = s_settings["sync_id"]
        settings_table["updated_at"] = s_settings["updated_at"]
        data["settings"] = settings_table

        deleted_table = {"objects": None, "categories": None, "updated_at": None}

        del_obj = DeletedItems.objects.filter(category__isnull=True).order_by(
            "-created_at"
        )
        del_cat = DeletedItems.objects.filter(objects_item__isnull=True).order_by(
            "-created_at"
        )
