...
from main.views.validators import (
    validate_chats,
    validate_votings,
...
class Synchronization(APIView):
    def get(self, request, format=None):
        ...
        if user_id:
            ...
            return JsonResponse(
                {"error": "Existing user id must be provided"}, safe=True, status=400
            )
        if museum_id:
            try:
            ...
            return JsonResponse(
                {"error": "Existing museum id must be provided"}, safe=True, status=400
            )

        foreign_colns = user.collections_set.exclude(
            objects_item__in=museum.objectsitem_set.all()
        )
        foreign_chats = user.chats_set.exclude(
            objects_item__in=museum.objectsitem_set.all()
        )
        ...
        if not settings:
            ...
            return JsonResponse(
                {"error": "museums settings must be defined"}, safe=True, status=400
            )

        return JsonResponse(
            serialize_synch_data(
                museum, user, settings, categories, f_musems_to_serialize
            ),
            safe=True,
        )
    ...
    def post(self, request, format=None):
        ...
        if user_id:
            try:
                ...
                return JsonResponse(
                    {"error": f"User id {user_id} does not exist {e.args}"},
                    safe=True,
                    status=400,
                )
        else:
        ...
        try:
            ...
            if isinstance(post_data, str):
                try:
                    ...
                    return JsonResponse(
                        {"Failed loading datato json format"}, safe=True, status=400
                    )

        except (json.JSONDecodeError, TypeError):
            logger.error(
                'json data with schema {"add": {}, "update": {},"delete": {}, "get": {} } must be transfered'
            )
            return JsonResponse(
                {
                    "error": 'json data with schema {"add": {}, \
        ...
        if get_values.get("categories"):
            if isinstance(get_values.get("categories"), list):
                ...
                return JsonResponse(
                    {"error": "Categories must be list"}, safe=True, status=400
                )

        logger.info(
            f'GET objects: {get_values.get("objects")}, \
        ...
        if chats:
            for chat in chats:
                ...
                validated_data, errors = validate_chats(
                    "add",
                    data,
                    user,
                    errors,
        ...
        if votings:
            for voting in votings:
                ...
                validated_data, errors = validate_votings(
                    "add",
                    data,
                    user,
                    errors,
        ...
        if collections:
            for collection in collections:
                ...
                validated_data, errors = validate_collections(
                    "add",
                    data,
                    user,
                    errors,
        ...
        if tours:
            for tour in tours:
                ...
                validated_data, errors = validate_tours(
                    "add",
                    data,
                    user,
                    errors,
        ...
        for name, lst in table.items():
            for item in lst:
                try:
                    ...
                    return JsonResponse({f"{name}": e.args}, safe=True, status=400)
        ...
        if up_collections:
            ...
            for collection in up_collections:
                ...
                ctgs = validated_data.pop("category", None)
                img = validated_data.pop("image", None)
                coltn = Collections.objects.filter(
                ...
                if coltn:
                    try:
                        ...
                        try:
                            ...
                            errors["update_errors"].append(
                                {
                                    "collection": f"collection image update failed: {e.args}"
                                }
                            )
                            logger.error(errors)
                            return JsonResponse(errors, safe=True)
        ...
        if up_user_data:
            ...
            if not user:
                errors["update_errors"].append(
                    {"user": "User device id and sync id does not match"}
                )
                logger.error(errors)
                return JsonResponse(errors, safe=True)
            else:
        ...
        return JsonResponse(
            serialize_synch_data(museum, settings=settings, categories=categories),
            safe=True,
        )
