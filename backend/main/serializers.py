...
class ObjectsItemSerializer(serializers.ModelSerializer):
    ...
    def get_semantic_relation(obj):
        relations = SemanticRelation.objects.filter(from_object_item=obj.id).annotate(
            object_item_id=F("to_object_item__sync_id")
        )
...
class MusemsTensorSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = MusemsTensor
        fields = (
            "mobile_tensor_flow_model",
            "mobile_tensor_flow_lables",
            "sync_id",
            "created_at",
            "updated_at",
        )
...
class ShortMuseumsSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Museums
        fields = ("sync_id", "created_at", "updated_at", "localizations")
...
def serialize_synch_data(
    museum, user=None, settings=None, categories=None, foreign_museums=None
):
    ...
    for category in categories:
        ...
        category_table["sync_object_ids"] = [
            str(i.objects_item.sync_id) for i in objects
        ]
    ...
    if settings:
        ...
        settings_table["predifined_objects"] = [
            str(i.predefined_object.sync_id)
            for i in settings.settingspredefinedobjectsitems_set.all()
        ]
        ...
        settings_table["predefined_avatars"] = [
            i["image"] for i in serialized_settings["predefined_avatars"]
        ]
