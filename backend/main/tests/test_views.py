...
class TestSynchronization(APITestCase):
    ...
    def test_synch_without_user_id(self):
        ...
        self.assertEqual(
            response.json(), {"error": "Existing user id must be provided"}
        )
    ...
    def test_create_user(self):
        ...
        response = self.client.get(
            self.url, {"user_id": self.t_user, "museum_id": self.museum_id}
        )
    ...
    def test_museums(self):
        ...
        self.assertEqual(img_types, ["logo", "1_map", "pnt"])
        ...
        self.assertIsInstance(
            museum["tours"][0]["localizations"][0]["description"], str
        )
        ...
        self.assertIsInstance(
            object_item["localizations"][0]["conversation"], (str, type(None))
        )
    ...
    def test_add_chats(self):
        self.client.get(self.url, {"user_id": self.t_user, "museum_id": self.museum_id})
        data = {"add": {"chats": [self.chat]}, "delete": {}, "get": {}, "update": {}}

        chats_count = Chats.objects.count()
        self.assertEqual(chats_count, 1)
        response = self.client.post(
            self.url_for_post, {"data": json.dumps(data)}, format="multipart"
        )
        self.assertEqual(response.status_code, 200)
        chats_count = Chats.objects.count()
    ...
    def test_add_tours(self):
        self.client.get(self.url, {"user_id": self.t_user, "museum_id": self.museum_id})
        data = {"add": {"tours": [self.tour]}, "delete": {}, "get": {}, "update": {}}
    ...
    def test_update_chats(self):
        self.client.get(
            self.url,
            {"user_id": self.t_user, "museum_id": self.museum_id},
            format="json",
        )
        data = {"add": {"chats": [self.chat]}, "delete": {}, "get": {}, "update": {}}
        ...
        data = {"add": {}, "delete": {}, "get": {}, "update": {"chats": [chat]}}
...
class TestFetch(APITestCase):
    ...
    def test_fetch_with_user_id_not_exist(self):
        response = self.client.get(
            self.url,
            {"user_id": "not_exist", "museum_id": self.museum_id},
            format="json",
        )
        self.assertEqual(response.status_code, 400)
