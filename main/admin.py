...
class ObjectsItemAdmin(nested_admin.NestedModelAdmin):
    ...
    def get_readonly_fields(self, request, obj=None):
        ...
        readonly_fields = super(ObjectsItemAdmin, self).get_readonly_fields(request, obj)
        if obj:
            return readonly_fields + ['museum']

        messages.warning(request, "Pay attention that you won't be able to change museum of the object \
    ...
    def save_model(self, request, obj, form, change):
        ...
        if len(request.FILES.getlist('photos_multiple')) > 10 or any(sizes):
            messages.warning(request, "Images number should not exceed 10 for one time and images size must be \
